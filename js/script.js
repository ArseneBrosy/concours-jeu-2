...
setInterval(() => {
  ...
  if (isReturningToTrack && timeRemainingToTrack > 0) {
    ...
    if (timeRemainingToTrack <= 0) {
      let point = (trackPos - 5) % TRACK.length;
      if (point < 0) {
        point += TRACK.length;
      }
      returnToTrack(point);
  ...
  //region Start line
  ctx.translate(TRACK[0][0] + camOffsetX, TRACK[0][1] + camOffsetY);
  ctx.rotate(startLineAngle * (Math.PI / 180));
  ctx.drawImage(START_SPRITE, -TRACK_WIDTH / 2, -TRACK_WIDTH * 150 / 250, TRACK_WIDTH, TRACK_WIDTH * 75 / 250);
  ctx.rotate(-startLineAngle * (Math.PI / 180));
  ctx.translate(-TRACK[0][0] - camOffsetX, -TRACK[0][1] - camOffsetY);
  //endregion
